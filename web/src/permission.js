...
// 白名单路由
const WHITE_LIST = ['Login', 'Init', 'Root']
...
router.beforeEach(async (to, from) => {
  ...
  if (WHITE_LIST.includes(to.name)) {
    // 如果访问根路径或初始化页面，先检查数据库状态
    if (to.name === 'Root' || to.name === 'Init') {
      console.log('检查数据库状态，当前路由:', to.name)
      try {
        const res = await checkDB()
        console.log('checkDB响应:', res)
        // 如果数据库已初始化，跳转到登录页面
        if (res.data && res.data.needInit === false) {
          console.log('数据库已初始化，跳转到登录页面')
          return { name: 'Login' }
        } else {
          console.log('数据库未初始化，needInit:', res.data?.needInit)
          // 如果数据库未初始化，跳转到初始化页面
          if (to.name === 'Root') {
            return { name: 'Init' }
          }
        ...
        console.log('检查数据库状态失败:', error)
        // 如果检查失败，根路径跳转到初始化页面，初始化页面继续显示
        if (to.name === 'Root') {
          return { name: 'Init' }
        }
      }
